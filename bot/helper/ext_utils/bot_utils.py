...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if download.status() not in [MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_PAUSED,
            ...
            msg += f"{get_progress_bar_string(download.progress())} <b>» {download.status()} | {download.speed()}</b>"
            ...
            if hasattr(download, 'seeders_num'):
                try:
                    msg += f" | {download.seeders_num()}/{download.leechers_num()}"
                except:
                    pass
        ...
        if config_dict['DELETE_LINKS']:
            msg += f" | #{download.extra_details['mode']}"
        else:
            msg += f" | #<a href='{download.message.link}'>{download.extra_details['mode']}</a>"
        msg += f"\n<b>User   </b>: {tag}"
    ...
    msg += f"\n<b>FREE: </b>{get_readable_file_size(disk_usage(config_dict['DOWNLOAD_DIR']).free)} | <b>UPTM: </b>{get_readable_time(time() - botStartTime)}"
